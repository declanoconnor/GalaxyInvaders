...
function starFieldGenerator.new(numberOfStars,theView,starSpeed)
    ...
    for i=0, numberOfStars do
		local star = display.newCircle(math.random(display.contentWidth), math.random(display.contentHeight), math.random(1,3))
...
function  starFieldGenerator:checkStarsOutOfBounds()
	for i=1, #self.allStars do
		if(self.allStars[i].y-100 > display.contentHeight) then
